...
trad_cultural_social <- subset(trad_cultural_social, n_obs != 224 & mutation_rate != 0 & mutation_rate != 0.2
                               & mutation_rate != 0.4 & mutation_rate != 0.6 & mutation_rate != 0.7 
                               & mutation_rate != 0.8 & mutation_rate != 0.9)
...
#                        mutation_rate != 0.3 & n_obs != 224)
trad_cultural_null <- subset(trad_cultural_null, n_obs != 224 & mutation_rate != 0 & mutation_rate != 0.2
                             & mutation_rate != 0.4 & mutation_rate != 0.6 & mutation_rate != 0.7 
                             & mutation_rate != 0.8 & mutation_rate != 0.9)
...
#                          mutation_rate != 0.3 & n_obs != 224)
trad_geneculture_social <- subset(trad_geneculture_social, n_obs != 224 & mutation_rate != 0 & mutation_rate != 0.2
                                  & mutation_rate != 0.4 & mutation_rate != 0.6 & mutation_rate != 0.7 
                                  & mutation_rate != 0.8 & mutation_rate != 0.9)
...
#                        mutation_rate != 0.3 & n_obs != 224)
trad_geneculture_null <- subset(trad_geneculture_null, n_obs != 224 & mutation_rate != 0 & mutation_rate != 0.2
                                & mutation_rate != 0.4 & mutation_rate != 0.6 & mutation_rate != 0.7 
                                & mutation_rate != 0.8 & mutation_rate != 0.9)
...
p <- list(
  ...
  survival = 0.999,                     # Baseline survival probability per time step
  ageing = 0.1,                         # Probability of staying in same age class = 1-a
  ...
  mutation_rate = 0.1,                  # Default mutation rate (overridden by grid), 
                                            # with heritability h = 1 - mutation_rate
...
param_variation_total <- expand.grid(
  ...
  mutation_rate = seq(0, 1, 0.1),                      # Genetic transmission fidelities
...
param_variation <- expand.grid(
  ...
  mutation_rate = c(0.1, 0.3, 0.5, 1),            # Key mutation rate values
...
# Create panel labels for parameter h (measure of heritability) facets
# These appear as headers in the faceted heatmaps
h_labs <- c()
for (i in 1:length(unique(param_variation$mutation_rate))){
  h_labs[i] <- paste("h =", 1-unique(param_variation$mutation_rate)[i])
}
names(h_labs) <- unique(param_variation$mutation_rate)
...
heatmap_cultural <- ggplot(prop_traditions_cultural, 
  ...
  facet_wrap(vars(mutation_rate), labeller = labeller(mutation_rate = h_labs), 
             ncol = length(h_labs)) +
