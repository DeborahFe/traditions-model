...
p <- list(
  ...
  mutation_rate = 0.1, ## with heritability h = 1 - mutation_rate
  n_obs = 6, ## maximal number of couples observed per female
  c = 0.7, ## copying rate
  n_steps_removed = 100) ## must be > 0

## dataframe for parameters variation
param_variation <- expand.grid(n_obs = c(seq(1, 30, 2), 224),
                               mutation_rate = seq(0, 1, 0.1),
                               c = seq(0.5, 1, 0.05))

param_variation_null <- expand.grid(n_obs = c(seq(1, 30, 2), 224),
                                    mutation_rate = seq(0, 1, 0.1),
                                    c = 0.5)
...
file_name_social_modelA <- "trad_social_modelA.csv"
file_name_null_modelA <- "trad_null_modelA.csv"
file_name_social_modelB <- "trad_social_modelB.csv"
file_name_null_modelB <- "trad_null_modelB.csv"
