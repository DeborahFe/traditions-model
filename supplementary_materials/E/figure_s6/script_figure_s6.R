...
# ==============================================================================
# This script generates Figure 11 (supplementary material) showing tradition 
...
p <- list(
  ...
  ageing = 0.9,                 # Per-timestep ageing probability (must be < 1)
...
param_variation <- expand.grid(n_obs = seq(1, 30, 2),                    
                               mutation_rate = 0.1,                       
                               c = seq(0.5, 1, 0.05),                    
                               ageing = c(0.1, 0.2, 0.3, 0.4)) # variation of the ageing rate
...
quantile95 <- function(nobs, mutation, ageing, df_null) {
  # Filter null model data for matching parameter combination
  df_subset <- df_null[which(df_null$n_obs == nobs & df_null$mutation_rate == mutation & df_null$ageing == ageing), ]
...
prop_traditions <- function(n_obs, c_index, mutation_rate, ageing, df_null, df_social) {
  ...
  quantile_95 <- quantile95(n_obs, mutation_rate, ageing, df_null)
...
col_names <- c("simu", "n_obs", "c", "mutation_rate", "ageing", "n_demos", 
...
ageing_labs <- c()
ageing_values <- 1 - unique(param_variation$ageing)
for (i in 1 : length(ageing_values)){
  ageing_labs[i] <- paste("a =", ageing_values[i])
}
names(ageing_labs) <- unique(param_variation$ageing)
...
prop_traditions_cultural_df$ageing <- as.factor(prop_traditions_cultural_df$ageing)
...
prop_traditions_geneculture_df$ageing <- as.factor(prop_traditions_geneculture_df$ageing)
...
heatmap_traditions_cultural <- ggplot(prop_traditions_cultural_df, aes(x = n_obs, y = c, fill = prop_traditions)) +
  ...
  facet_wrap(vars(ageing), labeller = labeller(ageing = ageing_labs), ncol = length(ageing_labs)) +
  theme_minimal() +
