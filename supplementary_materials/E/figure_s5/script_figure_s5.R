...
# ==============================================================================
# This script generates Figure 9 (supplementary material) showing tradition 
# existence across Cultural and Gene-Culture models, for different values of 
# the population carrying capacity K.
# ==============================================================================
...
param_variation <- expand.grid(n_obs = seq(1, 30, 2),                    
                               mutation_rate = 0.1,                       
                               c = seq(0.5, 1, 0.05),                    
                               K = c(250, 500, 1000, 2000)) # variation of the population carrying capacity
...
quantile95 <- function(nobs, mutation, K, df_null) {
  # Filter null model data for matching parameter combination
  df_subset <- df_null[which(df_null$n_obs == nobs & df_null$mutation_rate == mutation & df_null$K == K), ]
...
prop_traditions <- function(n_obs, c_index, mutation_rate, K, df_null, df_social) {
  ...
  quantile_95 <- quantile95(n_obs, mutation_rate, K, df_null)
...
col_names <- c("simu", "n_obs", "c", "mutation_rate", "K", "n_demos", 
...
K_labs <- c()
for (i in 1 : length(unique(param_variation$K))){
  K_labs[i] <- paste("K =", unique(param_variation$K)[i])
}
names(K_labs) <- unique(param_variation$K)
...
prop_traditions_cultural_df$K <- as.factor(prop_traditions_cultural_df$K)
...
prop_traditions_geneculture_df$K <- as.factor(prop_traditions_geneculture_df$K)
...
heatmap_traditions_cultural <- ggplot(prop_traditions_cultural_df, aes(x = n_obs, y = c, fill = prop_traditions)) +
  ...
  facet_wrap(vars(K), labeller = labeller(K = K_labs), ncol = length(K_labs)) +
  theme_minimal() +
