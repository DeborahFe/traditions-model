...
# ==============================================================================
# This script generates Figure 6 (supplementary material) showing tradition 
# existence across Cultural and Gene-Culture models, for different values of 
# the number of time steps removed before detecting traditions.
# ==============================================================================
...
param_variation <- expand.grid(n_obs = seq(1, 30, 2),                    
                               mutation_rate = 0.1,                       
                               c = seq(0.5, 1, 0.05),                    
                               n_steps_removed = c(25, 50, 100, 200))    
...
quantile95 <- function(nobs, mutation, n_steps_removed, df_null) {
  # Filter null model data for matching parameter combination
  df_subset <- df_null[which(df_null$n_obs == nobs & df_null$mutation_rate == mutation & df_null$n_steps_removed == n_steps_removed), ]
...
prop_traditions <- function(n_obs, c_index, mutation_rate, n_steps_removed, df_null, df_social) {
  ...
  quantile_95 <- quantile95(n_obs, mutation_rate, n_steps_removed, df_null)
...
col_names <- c("simu", "n_obs", "c", "mutation_rate", "n_steps_removed", "n_demos", 
...
for (i in 1 : nrow(param_variation)) {
  ...
  prop_traditions_cultural_df[i, "quantile_95"] <- quantile95(param_variation[i, "n_obs"], 
                                                              ...
                                                              param_variation[i, "n_steps_removed"], 
                                                              trad_null_cultural)
...
n_steps_removed_labs <- c()
for (i in 1 : length(unique(param_variation$n_steps_removed))){
  n_steps_removed_labs[i] <- paste("steps removed =", unique(param_variation$n_steps_removed)[i])
}
names(n_steps_removed_labs) <- unique(param_variation$n_steps_removed)
...
prop_traditions_cultural_df$n_steps_removed <- as.factor(prop_traditions_cultural_df$n_steps_removed)
...
prop_traditions_geneculture_df$n_steps_removed <- as.factor(prop_traditions_geneculture_df$n_steps_removed)
...
heatmap_traditions_cultural <- ggplot(prop_traditions_cultural_df, aes(x = n_obs, y = c, fill = prop_traditions)) +
  ...
  facet_wrap(vars(n_steps_removed), labeller = labeller(n_steps_removed = n_steps_removed_labs), ncol = length(n_steps_removed_labs)) +
  theme_minimal() +
