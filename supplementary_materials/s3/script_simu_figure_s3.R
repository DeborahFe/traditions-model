...
p <- list(
  ...
  initial_pref_frequency = 0.5, # initial probability that females prefer trait 1
  n_matings = 100, ## maximum number of matings for males at every time step, must be > 0
  mutation_rate = 0.4, ## with heritability = 1 - mutation_rate
  n_obs = 7, ## maximal number of couples observed per female
  c = 0.7, ## copying rate
  n_steps_removed = 1) ## must be > 0
...
n_simus_null <- nrow(param_variation_null) * n_replicates

## number of time steps
n_steps <- 500

## file name to store results
file_name_social_modelA <- "first_trad_social_modelA.csv"
file_name_null_modelA <- "first_trad_null_modelA.csv"
file_name_social_modelB <- "first_trad_social_modelB.csv"
file_name_null_modelB <- "first_trad_null_modelB.csv"


#### functions to run simulations ####


init_pop_A <- function(parameters) {
  males <- matrix(NA, nrow = parameters$K, ncol = 4) ## age / trait / neutral trait / alive or not / nb of matings    
  females <- matrix(NA, nrow = parameters$K, ncol = 7) ## age / trait / neutral trait / alive or not / type observed / type preferred / ID of male chosen
  ...
  if (n_tot_females > 0) {
    ...
    # females[pos, "preferred"] <- rbinom(n_tot_females, size = 1, prob = parameters$initial_trait_frequency)
  ...
  ## Initialization of a preference for reproductive females
  pos_fR <- which(reproductive[females[, "age"]]) # positions of reproductive females
  if (length(pos_fR) > 0) {
    females[pos_fR, "preferred"] <- rbinom(length(pos_fR), size = 1, prob = parameters$initial_pref_frequency)
  }
  
  ## Initialization of male chosen for reproductive females (based on their initialized preference)
  # mating based on conformist capacities only (mating independent of male types frequencies, if at least one male of each type exists)  
  
  pos_f_non_mated <- pos_fR # females who have to choose a mate (reproductive females)
  pos_mR <- which(reproductive[males[, "age"]]) # reproductive males
  pos_m_non_mated <- pos_mR # males who still have matings to make
  
  pos_f_0 <- which(females[pos_f_non_mated, "preferred"] == 0) # positions of females with preference 0
  pos_f_1 <- which(females[pos_f_non_mated, "preferred"] == 1) # positions of females with preference 1
  pos_m_0 <- which(males[pos_m_non_mated, "trait"] == 0) # positions of males with trait 0
  pos_m_1 <- which(males[pos_m_non_mated, "trait"] == 1) # positions of males with trait 1
  pos_m_met_0 <- pos_m_met_1 <- pos_m_error_0 <- pos_m_error_1 <- NULL # vector for positions of males met by females and accepted or rejected
  
  
  if (length(pos_m_0) == 0 & length(pos_m_1) > 0) { # if the only available type is type 1
    if (parameters$c == 1) { # if c = 1: females who prefer 1 choose 1, females who prefer 0 don't mate
      if (length(pos_m_1) == 1) {
      ...
      females[pos_f_non_mated[pos_f_1], "ID_father"] <- pos_m_met_1
      females[pos_f_non_mated[pos_f_0], "ID_father"] <- NA
    } else { # if c < 1 : all females choose type 1
      if (length(pos_m_1) == 1) {
        pos_m_met <-  rep(pos_m_non_mated, length(pos_f_non_mated))
      } else {
        pos_m_met <- sample(pos_m_non_mated, length(pos_f_non_mated), replace = TRUE)
        females[pos_f_non_mated, "ID_father"] <- pos_m_met
      }
    }
  }
  if (length(pos_m_1) == 0 & length(pos_m_0) > 0) { # if the only available type is type 0
    if (parameters$c == 1) { # if c = 1: females who prefer 0 choose 0, females who prefer 1 don't mate
      if (length(pos_m_0) == 1) {
      ...
      females[pos_f_non_mated[pos_f_0], "ID_father"] <- pos_m_met_0
      females[pos_f_non_mated[pos_f_1], "ID_father"] <- NA
    } else { # if c < 1 : all females choose type 0
      if (length(pos_m_0) == 1) {
        pos_m_met <-  rep(pos_m_non_mated, length(pos_f_non_mated))
      } else {
        pos_m_met <- sample(pos_m_non_mated, length(pos_f_non_mated), replace = TRUE)
  ...
  if (length(pos_m_0) > 0 & length(pos_m_1) > 0) { # if both types of males are available
    # mate choice for females with preference 0 :
    if (length(pos_m_0) == 1) {
      pos_m_met_0 <- rep(pos_m_non_mated[pos_m_0], length(pos_f_0))
    } else {
      pos_m_met_0 <- sample(pos_m_non_mated[pos_m_0], length(pos_f_0), replace = TRUE)
    }
    chosen_0 <- rbinom(length(pos_f_0), size = 1, prob = parameters$c) # males are accepted (1) or not (0)
    occurences_0 <- table(factor(chosen_0, levels = 0 : 1))
    if (occurences_0["1"] > 0) {
      females[pos_f_non_mated[pos_f_0[chosen_0 == 1]], "ID_father"] <- pos_m_met_0[chosen_0 == 1]
    }
    if (occurences_0["0"] > 0) { # females who don't accept their male with trait 0 get a male with trait 1
      if (length(pos_m_1) == 1) {
      ...
      females[pos_f_non_mated[pos_f_0[chosen_0 == 0]], "ID_father"] <- pos_m_error_0
    }
    # mate choice for females with preference 1 :
    if (length(pos_m_1) == 1) {
      pos_m_met_1 <- rep(pos_m_non_mated[pos_m_1], length(pos_f_1))
    } else {
      pos_m_met_1 <- sample(pos_m_non_mated[pos_m_1], length(pos_f_1), replace = TRUE)
    }
    chosen_1 <- rbinom(length(pos_f_1), size = 1, prob = parameters$c) # males are accepted (1) or not (0)
    occurences_1 <- table(factor(chosen_1, levels = 0 : 1))
    if (occurences_1["1"] > 0) {
      females[pos_f_non_mated[pos_f_1[chosen_1 == 1]], "ID_father"] <- pos_m_met_1[chosen_1 == 1]
    }
    if (occurences_1["0"] > 0) { # females who don't accept their male with trait 0 get a male with trait 1
      if (length(pos_m_0) == 1) {
      ...
      females[pos_f_non_mated[pos_f_1[chosen_1 == 0]], "ID_father"] <- pos_m_error_1
    }
  }
...
init_pop_B <- function(parameters) {
  males <- matrix(NA, nrow = parameters$K, ncol = 4) ## age / trait / neutral trait / alive or not / nb of matings    
  females <- matrix(NA, nrow = parameters$K, ncol = 7) ## age / trait / neutral trait / alive or not / type observed / type preferred / ID of male chosen
  colnames(males) <- c("age", "trait", "neutral", "alive")
  colnames(females) <- c("age", "trait", "neutral", "alive", "observed", "preferred", "ID_father")
  ...
  ## Initialization of male chosen for reproductive females (based on their initialized preference)
  # mating based on conformist capacities AND encounters with males (mating dependent of male types frequencies)  
  ...
  while (length(pos_f_non_mated) > 0 && length(pos_m_non_mated) > 0) { # as long as there are males and females available to mate
    ...
    if (parameters$c == 1 & !any(females[pos_f_non_mated, "preferred"] %in% males[pos_m_non_mated, "trait"])) { # if there is no preference that corresponds to the available traits
      break }
    ...
    pr <- ifelse(males[pos_m_met, "trait"] == females[pos_f_met, "preferred"], # probability to accept the sampled (met) male, for each female
                 parameters$c,
                 1 - parameters$c)
    
    if(any(is.na(males[pos_m_met, "trait"]))) stop("NA in met males traits")
    ...
    if (any(chosen == 1)) { # if matings occur, store the choice of females and update the vectors of males and females still available for mating
      females[pos_f_met[chosen == 1], "ID_father"] <- pos_m_met[chosen == 1]
      pos_f_non_mated <- pos_fR[is.na(females[pos_fR, "ID_father"])] # update positions of females without a mate
...
dynamics_A <- function(tmax, parameters, return_tradition_only = FALSE) { # if return_tradition_only = FALSE, store raw temporal results, if not, store only one tradition (duration) per simulation
  ## initialization
  pop <- init_pop_A(parameters)
  ...
  for(g in 1 : tmax) {
    ...
    ## mating based on preferences without competition among males
    ## in this version of mate choice, females always meet a male of their preference (if at least one is available) and choose him with proba c
    ...
    pos_f_0 <- which(pop$females[pos_f_non_mated, "preferred"] == 0) # positions of females with preference 0
    pos_f_1 <- which(pop$females[pos_f_non_mated, "preferred"] == 1) # positions of females with preference 1
    pos_m_0 <- which(pop$males[pos_m_non_mated, "trait"] == 0) # positions of males with trait 0
    pos_m_1 <- which(pop$males[pos_m_non_mated, "trait"] == 1) # positions of males with trait 1
    pos_m_met_0 <- pos_m_met_1 <- pos_m_error_0 <- pos_m_error_1 <- NULL # vector for positions of males met by females and accepted or rejected
    ...
    if (length(pos_m_0) == 0 & length(pos_m_1) > 0) { # if the only available type is type 1
      if (parameters$c == 1) { # if c = 1: females who prefer 1 choose 1, females who prefer 0 don't mate
        ...
        pop$females[pos_f_non_mated[pos_f_0], "ID_father"] <- NA
      } else { # if c < 1 : all females choose type 1
        if (length(pos_m_1) == 1) {
          pos_m_met <-  rep(pos_m_non_mated, length(pos_f_non_mated))
        } else {
    ...
    if (!return_tradition_only) { # outputs if we store raw temporal results
      ...
      if (g == parameters$n_steps_removed) { # we start to compute the duration of the tradition here (initialized at duration 0)
        mean_choice <- mean(pop$males[pop$females[, "ID_father"], "trait"], na.rm = TRUE)
        trad_duration <- 0
        if (mean_choice > 0.5) { # we will store if this is a tradition for trait 0 or a tradition for trait 1
          trad_for_trait <- 1
        } else {
          trad_for_trait <- 0
        }
      ...
      if (g > parameters$n_steps_removed) { # start computing the duration of the tradition and return results if the tradition ends
        ...
        if (xor(mean_choice > 0.5, mean_choice_old > 0.5)) return(c(last_time_step = g, mean_choice_last_step = mean_choice_old, 
                                                                    ...
                                                                    tradition_duration = trad_duration, tradition_for_trait = trad_for_trait))
...
dynamics_B <- function(tmax, parameters, return_tradition_only = FALSE) { # if return_tradition_only = FALSE, store raw temporal results, if not, store only one tradition (duration) per simulation
  ## initialization
  pop <- init_pop_B(parameters)
  ...
  for(g in 1 : tmax) {
    ...
    ## mating based on conformity capacities AND encounters with males (mating dependent of male types frequencies)
    pop$females[, "ID_father"] <- NA # reset the column of chosen mates from the previous time step
    pos_f_non_mated <- which(pop$reproductive[pop$females[, "age"]] & is.na(pop$females[, "ID_father"])) # females who have to choose a mate
    pos_m_non_mated <- pos_mR # males who still have matings to make
    nb_matings_males <- rep(parameters$n_matings, length(pos_mR)) # number of matings to make for each male
    ...
    while (length(pos_f_non_mated) > 0 && length(pos_m_non_mated) > 0) { # as long as there are males and females available to mate
      # stop the while loop if c = 1 and all females preferences are different from males traits (no choice possible, the while loop would never end)
      if (parameters$c == 1 & !any(pop$females[pos_f_non_mated, "preferred"] %in% pop$males[pos_m_non_mated, "trait"])) { # if there is no preference that corresponds to the available traits
        break }
...
parameters_exploration <- function(dynamics_type, param_grid, parameters, file_name, n_replicates = 10, tmax = 200) {
  ...
  for (i in 1 : nrow(param_grid)) {
    ...
    for (r in 1 : n_replicates) {
      simu <- try(dynamics_type(tmax, parameters, return_tradition_only = TRUE)) # run the simulation
...
# social model
start <- Sys.time()
parameters_exploration(dynamics_A, param_variation, p, file_name_social_modelA, n_replicates, tmax = n_steps)
end <- Sys.time()
print(end - start)
# null model
start <- Sys.time()
parameters_exploration(dynamics_A, param_variation_null, p, file_name_null_modelA, n_replicates, tmax = n_steps)
end <- Sys.time()
print(end - start)
...
# social model
start <- Sys.time()
parameters_exploration(dynamics_B, param_variation, p, file_name_social_modelB, n_replicates, tmax = n_steps)
end <- Sys.time()
print(end - start)
# null model
start <- Sys.time()
parameters_exploration(dynamics_B, param_variation_null, p, file_name_null_modelB, n_replicates, tmax = n_steps)
end <- Sys.time()
print(end - start)
